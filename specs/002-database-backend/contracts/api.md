...
Write endpoints and the MCP trip-intelligence interface require a bearer token:
```
Authorization: Bearer <API_SECRET_KEY>
```
Unauthenticated write requests return **401**. The `API_SECRET_KEY` is set in `.env`. A single shared secret is used across all authenticated surfaces — no separate MCP key.
...
Creates a new trip. The `id` is auto-generated by the backend by slugifying the `title` (lowercase, hyphens). If a unique title produces a slug that collides with an existing id, the backend appends an incrementing counter (e.g. `new-adventure-2029-2`).
...
**Response 409**: A trip with the same `title` already exists.
...
Returns service health status. Used by Docker Compose health checks. No authentication required.
...
Returns all region records. No authentication required.

**Query parameters**:

| Parameter | Type | Description |
|-----------|------|-------------|
| `country` | string | Optional filter by country name (exact match) |
...
### `POST /regions/end-date` *(auth required)*

> **Status**: Contract PENDING — storage model and migration must be decided in a design meeting before this endpoint is implemented. See FR-019 in spec.md and T037 in tasks.md. The shape below is a draft sketch only.

Records the end date of a geographic region within a trip. All parameters are optional and resolve to smart defaults.

**Request body** (all fields optional):
```json
{
  "trip": "earth-club-sandwich-2027",
  "region": "MDE",
  "date": "2027-06-15"
}
```
...
Defaults:
- `trip` → most recently created trip (by `created_at`)
- `region` → most recently active `region_code` seen in that trip's stops
- `date` → today's date

**Response 200**: TBD — to be defined alongside the storage model.
**Response 401**: Missing or invalid bearer token.
**Response 404**: Resolved trip or region not found.

---

## MCP Trip Intelligence Interface

> **Status**: Contract PENDING — research and contract definition are prerequisites for implementation (FR-023). Contract will live in `contracts/mcp.md`. Auth model resolved: same `API_SECRET_KEY` bearer token as write endpoints.
