...
- Q: Are `region_code` and `nearest_airport_iata` the same concept? → A: Yes — merge into one. `region_code` holds the computed IATA code; `nearest_airport_iata` is removed from the schema. (The IATA value is computed by the pile-app (`003-ingestion-pipeline`) and persisted to the pile; the eventual writer into this column is the planned **bridge-app** — see [`docs/roadmap.md`](../../docs/roadmap.md). The column itself lives in this spec.)
...
- What happens when the database is unavailable when a backend request fires? (Read endpoints return 503 with a structured error; write endpoints return the same. The pile-app (`003-ingestion-pipeline`) does not write to this DB — its output is the pile, consumed by a future bridge-app — so its behavior is unaffected by this DB's availability.)
...
> Pile-app-specific edge cases (instagrapi sessions, location inference, multi-target scrapes, anti-throttle, Substack feed availability, etc.) live in `003-ingestion-pipeline/spec.md`. Bridge-app concerns (multi-trip date overlap, Substack→stop linkage, etc.) move into the planned bridge-app spec — see [`docs/roadmap.md`](../../docs/roadmap.md). The Graph API fallback was retired in 2026-05-25; no successor.
...
- **FR-003**: The `stops` table MUST store: `id`, `trip_id` (foreign key → trips), `date`, `location` (human-readable string), `lat` (decimal), `lng` (decimal), `status` (`visited` | `planned`), `region_code` (3-letter IATA code of the nearest international airport within the same country as the stop; used for sidebar region grouping; the IATA value is computed by the pile-app (`003-ingestion-pipeline`) and persisted to the pile, then written into this column by the planned bridge-app — see [`docs/roadmap.md`](../../docs/roadmap.md)), `post_type` (`instagram` | `substack` | `planned`), `sequence_order` (integer for ordered rendering), `caption` (nullable text, used for planned stops only), `created_at`.
...
> Pile-app FRs (FR-016 through FR-022 for Instagram, FR-023 through FR-026 for Substack, plus FR-041 / FR-044) live in `003-ingestion-pipeline/spec.md`. FR-040 (trip auto-assignment by date range) and FR-027 (Substack post → stop linkage) were moved out of 003 during its 2026-05-27 re-author — both are cross-source operations that belong to the planned bridge-app (see [`docs/roadmap.md`](../../docs/roadmap.md)). FR-043 (Graph API fallback) was retired 2026-05-25.

#### Security

- **FR-028**: All credentials and secrets (`API_SECRET_KEY`, database credentials in `DATABASE_URL`) MUST be read from environment variables at startup and MUST NOT be hard-coded anywhere in the source. (Pile-app secrets — Instagram crawler credentials, Anthropic key for geocoding, Substack RSS URL — are specified in `003-ingestion-pipeline`. The Graph API token was retired 2026-05-25; SMTP for session alerts was dropped in the 2026-05-27 re-author.)
...
- **Stop**: A single itinerary entry linked to exactly one trip. Stores date, location string, decimal coordinates, status (`visited` | `planned`), `region_code` (IATA code; produced by the pile-app and written into this column by the planned bridge-app), post type, optional planned-stop caption, and sequence order. Mirrors the `Stop` interface in `src/data/types.ts`.
- **Instagram Post**: Rich content record linked to one stop. Stores the Instagram platform id, shortcode, relative media path, caption text, and original timestamp. Mirrors the `InstagramPost` interface in `src/data/types.ts`, extended with `instagram_id` and `shortcode`. Populated by the planned bridge-app, which consumes the pile-app's raw output (see [`docs/roadmap.md`](../../docs/roadmap.md)).
- **Substack Post**: Long-form article record linked to one stop (or unlinked pending manual assignment). Stores the Substack post identifier, title, subtitle, body, and publication date. Mirrors the `SubstackPost` interface in `src/data/types.ts`, extended with `substack_id` and `published_at`. Populated by the planned bridge-app, which handles Substack→stop linkage as a cross-source operation.
...
> Pile-app assumptions (personal Instagram account, Substack feed availability, LLM-based IATA determination, instagrapi session management) live in `003-ingestion-pipeline/spec.md`. SMTP for session alerts and the Graph API fallback were retired in the 2026-05-25 and 2026-05-27 amendments respectively. Bridge-app assumptions (cross-source trip auto-assignment, Substack→stop linkage) will land in the planned bridge-app spec.
