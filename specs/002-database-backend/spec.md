...
- Q: What storage model should persist `POST /regions/end-date` data? → A: Deferred — a proper design meeting is required before any decision. Endpoint MUST NOT be implemented until the design is decided and recorded in `specs/002-database-backend/contracts/api.md`.
- Q: How is `trips.id` generated for `POST /trips`? → A: Auto-generated by the backend from the `title` field (slugify: lowercase, hyphens). Caller does not provide the id.
- Q: What authentication model should the MCP trip-intelligence interface use? → A: Same `API_SECRET_KEY` bearer token as the write endpoints — no separate credential.
- Q: How should `PUT /trips/:id` handle concurrent edits? → A: Last-write-wins — no conflict detection or optimistic locking for v1.
- Q: What happens on a slug collision in `POST /trips`? → A: 409 Conflict if the title itself is a duplicate. If a unique title generates a slug that collides with an existing id, auto-suffix with an incrementing counter (e.g., `my-trip-2`).
...
- What happens when a `POST /trips` request is made with a `title` that already exists? (The API returns 409 Conflict with a structured error body — title must be unique.) What if a unique title produces a slug that collides with an existing `id`? (The backend auto-suffixes the slug with an incrementing counter, e.g., `earth-sandwich-2015-2`, and proceeds normally.)
...
- **FR-017**: `POST /trips` MUST create a new trip from `title`, `description`, `start_date`, and `end_date`. The `id` MUST be auto-generated by the backend by slugifying the `title` (lowercase, hyphens; e.g., "Earth Sandwich 2015" → `earth-sandwich-2015`). If a trip with the same `title` already exists, the endpoint MUST return 409 Conflict. If a unique title produces a slug that collides with an existing `id` (e.g., due to punctuation differences), the backend MUST auto-suffix with an incrementing counter (e.g., `earth-sandwich-2015-2`). On success it MUST return HTTP 201 and the generated `id`. This endpoint MUST require authorization (see FR-026).
- **FR-018**: `PUT /trips/:id` MUST support partial updates to a trip's `title`, `description`, `start_date`, and `end_date`. Omitted fields MUST remain unchanged. On success it MUST return HTTP 200 and the updated trip. This endpoint MUST require authorization (see FR-026). Concurrent edits use last-write-wins semantics — no optimistic locking or conflict detection is required for v1.
...
- **FR-023**: The interface contract (available tools, inputs, outputs, external data sources used) MUST be documented in `specs/002-database-backend/contracts/` before implementation begins. A research phase evaluating available event data sources MUST be completed and documented in `specs/002-database-backend/research.md` before the contract is finalized. The authentication model is resolved: the MCP interface MUST require the same `API_SECRET_KEY` bearer token used by the write API (see FR-026); no separate credential is introduced.
