...
- **FR-051**: The system MUST cluster region markers at the global trip overview level for qualifying countries. A cluster is shown when two or more region markers from the same country fall within the clustering threshold at the current zoom level, EXCEPT for the following excluded countries: United States, Canada, and China (markers from these countries always render individually regardless of proximity). A cluster indicator MUST display the count of grouped markers. Clicking a cluster MUST zoom the map in to a level where the individual pins separate and become individually clickable; no spiderfy or in-place expansion is used. Clustering applies only at the trip overview zoom level; region drill-down always shows individual stop markers.
