...
| `GET /projects/new` | Create form: name, pile directory, file selection, target descriptor, target-cred-env, pile-sample, force checkbox — mirroring `project create` inputs. |
| `POST /projects` | Two actions on one route. `action=list_files`: validates the pile directory and re-renders the form with its files as checkboxes (all pre-checked) — no state created. `action=create` (default): same core flow as `project create` (validate-then-materialize, FR-008 parity) with the checked files as the frozen selection (no checkboxes rendered yet ⇒ `all`). Success → redirect to the dashboard with the FR-007 validation report rendered inline. Failure (missing env var, unreadable/missing pile file, unreachable target, duplicate name) → form re-rendered with the inline error; no state created. |
...
| `GET /projects/{name}/edit` | Update form: pile directory, file selection (directory's current files as checkboxes, the frozen selection pre-checked), pile-sample, target-cred-env. |
