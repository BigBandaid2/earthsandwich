...
- Q: Are `region_code` and `nearest_airport_iata` the same concept? → A: Yes — merge into one. The ingestion pipeline (Airlabs/Claude) writes the computed IATA code directly to `region_code`; `nearest_airport_iata` is removed from the schema.
...
2. **Given** an Instagram post has a structured location tag, **When** it is ingested, **Then** the location name, latitude, longitude, and `region_code` (IATA code of the nearest in-country international airport) are stored in the database.
3. **Given** an Instagram post has no location tag, **When** it is ingested, **Then** Claude is called with the post caption and image to infer the location and IATA code, and the result is stored.
...
- What happens when the Airlabs IATA lookup returns no airport within the same country? (`region_code` is left null and a warning is logged; processing continues for the remaining posts.)
...
- What happens when an Instagram post's location tag provides a name but no coordinates? (The location name is stored; lat/lng are left empty. Claude is not called as a fallback when a structured location name already exists.)
...
- **FR-019**: For each new post the job MUST attempt to read the structured location tag (name, lat, lng, and country code) directly from the instagrapi post object. If a location name is present it MUST be used as-is without calling Claude. The job MUST then call the Airlabs API (or equivalent airport-lookup API configured via `AIRPORT_API_KEY`) to find the nearest international airport IATA code within the same country as the location; the result is stored in `stops.region_code`. If the airport lookup fails, `region_code` is left null and a warning is logged; processing MUST continue.
- **FR-020**: When instagrapi returns no location tag, the job MUST call Claude (`claude-opus-4-5`) with the post caption, the base64-encoded image (for IMAGE posts), and the locations of up to the 5 most-recently-processed posts as context. Claude MUST return a JSON object with `location`, `lat`, `lng`, and `region` (the IATA code of the nearest international airport within the same country). The `region` value is stored in `stops.region_code`. If the response is malformed JSON the raw text MUST be stored as the location name with empty coordinates and a null `region_code`.
...
- **FR-028**: All credentials and secrets (`INSTA_USERNAME`, `INSTA_PASSWORD`, `ANTHROPIC_API_KEY`, `SUBSTACK_RSS_URL`, `API_SECRET_KEY`) MUST be read from environment variables at startup and MUST NOT be hard-coded anywhere in the source.
...
- Region computation (grouping stops by nearest airport) remains a build-time or backend-time concern and is not changed by this feature; region codes are stored on each stop record.
...
- The Airlabs API (or equivalent) is used for IATA airport lookups; a valid `AIRPORT_API_KEY` must be configured for `region_code` to be populated automatically during ingestion. If the key is absent or the API is unreachable, `region_code` is stored as null and no error is raised at the stop level.
- `instagrapi` location objects include a country code that can be used directly to scope the nearest-airport query; a separate geocoding API is not required for the country determination step.
