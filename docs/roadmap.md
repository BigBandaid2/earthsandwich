...
| `004-test-and-cicd` | — | (project-wide tooling) | Project-wide test suite + GitHub Actions (per-PR unit lane, nightly integration lane) + deployment / domain pipeline (TLS, secrets, log aggregation; references [OCS-69](https://datacommlab.atlassian.net/browse/OCS-69)). Kernel in `pile-app/tests/`. | Next sprint or two. | [↓](#004-test-and-cicd) |
